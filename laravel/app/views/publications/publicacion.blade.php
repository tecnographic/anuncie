		...
		<div class="col-xs-12 formPub contAnaranjado @if(Session::has('error')) mostrar @endif" style="margin-top:2em;">
			...
			@if ($tipo == 'lider')
				...
				<div class="col-xs-12">
					...
					{{ Form::text('pagina',Input::old('pagina'),array('class' => 'form-control','placeholder' => 'http://example.com')) }}
			...
			@elseif($tipo == 'casual')	
				...
				<div class="col-xs-12">
					...
					@if ($errors->has('input'))
						 ...
						 @endforeach
					@elseif(Session::has('inputError'))
						<div class="alert alert-danger">
					 		<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
					 		<p class="textoPromedio">{{ Session::get('inputError') }}</p>
					 	</div>
					@endif
