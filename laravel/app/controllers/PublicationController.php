		...
		if ($pub->tipo == 'Lider') {
			if ($input['cat'] != $pub->categoria && !empty($input['cat'])) {
				...
				}else
				{
					...
					$img = Image::make('images/pubImages/'.Auth::user()['username'].'/'.$img2->getClientOriginalName());
		            $blank = Image::make('images/blank.jpg');
		            if ($img->width() > $img->height()) {
			        	$img->widen(401);
			        }else
			        ...
			        ->blank->insert($mark,'center')
		           	->interlace()
		            ->save('images/pubImages/'.Auth::user()['username'].'/'.$img2->getClientOriginalName());
					$pub->img_2 = Auth::user()['username'].'/'.$img2->getClientOriginalName();
