...
class UserController extends BaseController {
	...
	*/
	...
	public function postProfile()
	{
		...
		if ($user->state != $input['department']) {
			$user->state = $input['department'];
		}
		if ($user->phone != $input['phone']) {
			$user->phone = $input['phone'];
		}
		if ($user->pag_web != $input['pag_web']) {
			$user->pag_web = $input['pag_web'];
		}
		if ($user->postal_cod != $input['postal_cod']) {
			$user->postal_cod = $input['postal_cod'];
		}
		if ($user->dir != $input['direccion']) {
		...
		{
			$data = array(
				'link' => 'anuncie24.com/inicio/contacto'
			);
			Mail::send('emails.modify', $data, function ($message) use ($input,$email){
			    $message->subject('Correo de cambio de perfil anuncie24.com');
			    $message->to($email);
			});
	...
	public function postPublicationLider()
	{
		function chequear($publication,$img1,$donde)
		{
			if (file_exists('images/pubImages/'.Auth::user()['username'].'/'.$img1->getClientOriginalName())) {
		        ...
		        $mark = Image::make('images/logo.png')->widen(50);
			...
			}else
			{
		        ...
		        $mark = Image::make('images/logo.png')->widen(100);
		        $blank->insert($img,'center');
		        $blank->insert($mark,'center')
	           	->interlace()
	...
	{
		$title ="Publicación HABITUAL | anuncie24.com";
		$marcas = Marcas::all();
	...
	{
		$title ="Cambiar contraseña | anuncie24.com";
	...
	public function postChangePass()
	{
		...
		if (!Hash::check($input['old'], $user->password)) {
			Session::flash('error', 'la contraseña vieja es incorrecta');
		...
		}else
		{
			...
			if ($user->save()) {
				...
				Mail::send('emails.passNew', $data, function ($message) use ($newPass,$email){
					    $message->subject('Correo de cambio de contraseña anuncie24.com');
					    $message->to($email);
					});
	...
	{
		$title = "Mis compras | anuncie24.com";
	...
	{
		$title = "Mis ventas | anuncie24.com";
		$compras = Compras::join('publicaciones','publicaciones.id','=','compras.pub_id')
	...
	{
		$title = "Mi reputación|anuncie24.com";
		$compras = Compras::join('publicaciones','publicaciones.id','=','compras.pub_id')
		->join('usuario','usuario.id','=','publicaciones.user_id')
		->groupBy('compras.pub_id')
		->where('compras.user_id','=',Auth::id())
		->paginate(10,array(
			'compras.id',
			'compras.valor_vend',
