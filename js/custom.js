...
$('.btn-elim-pub').click(function(event) {
	...
	$('#eliminarPublicacionModal').click(function(event) {
		$(this).prop('disabled', true)
		$.ajax({
			url: 'eliminar-publicacion/enviar',
			type: 'POST',
			dataType: 'json',
			data: {'id': id},
			success:function(response){
...
jQuery(document).ready(function($) {
	function verificarComentario()
	{
		...
		$.ajax({
			//casa
			url: ' http://localhost/car2/public/verificar-comentarios',
			//trabajo
			//url: ' http://localhost/prueba/car2/public/verificar-comentarios',
			type: 'GET',
			success:function(response)
			{
				if(response != 0)
